...
handlers._tokens.post = function (data, callback) {
  ...
  if (phone && password) {
    ...
    _data.read("users", phone, function (err, userData) {
      _performance.mark("user lookup compeleted");
      if (!err && userData) {
        // Hash the sent password, and compare it to the password stored in the user object
        _performance.mark("beginning password hashing");
        var hashedPassword = helpers.hash(password);
        _performance.mark("password hashing compeleted");
        if (hashedPassword == userData.hashedPassword) {
          ...
          _data.create("tokens", tokenId, tokenObject, function (err) {
            _performance.mark("storing token compeleted");

            // Gather all the measurement
            _performance.measure(
              "Beginning to end",
              "entered function",
              "storing token compelete"
            );
            _performance.measure(
            ...
            _performance.measure(
              ...
              "user lookup compelete"
            );
            _performance.measure(
              "Password hashing",
              "beginning password hashing",
              "password hashing compelete"
            );
            _performance.measure(
              "Token data creation",
            ...
            measurments.forEach(function (measurement) {
              debug(
                "\x1b[33m%s\x1b[0m",
            ...
            if (userChecks.length < config.maxChecks) {
              ...
              dns.resolve(hostName, function (err, records) {
                if (!err && records) {
                  ...
                  callback(400, {
                    Error:
                      "The hostname of the URL entrered did not resolve to any DNS entries",
                  });
                }
              });
            } else {
