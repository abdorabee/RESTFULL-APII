...
cli.responders.listChecks = function (str) {
  _data.list("checks", function (err, checkIds) {
    if (!err && checkIds && checkIds.length > 0) {
      cli.verticalSpace();
      checkIds.forEach(function (checkIds) {
        _data.read("checks", checkIds, function (err, checkData) {
          var includeCheck = false;
          var lowerString = str.toLowerCase();

          // Get the state, deault to down
          var state =
          ...
          // if the user has specified the state ,or hasn't specified any state, include the check accordingcly
          if (
